...
export default function ImportPage() {
  ...
  const extractAllVocab = useCallback(async () => {
    const eligible = docs.filter((d) => d.processing_status === 'done')
    ...
    for (let i = 0; i < eligible.length; i++) {
      ...
      await extractVocab(eligible[i].id)
    }
    setBulkProgress(null)
    setExtractingAll(false)
  }, [docs, extractVocab])

  const analyzeAllReadiness = useCallback(async () => {
    const eligible = docs.filter((d) => d.processing_status === 'done' && d.word_count > 0)
    if (!eligible.length) return
    setError(null)
    ...
    for (let i = 0; i < eligible.length; i++) {
      setBulkProgress({ current: i + 1, total: eligible.length })
      try {
        await window.db.documents.analyzeReadiness(eligible[i].id)
        const updated = await window.db.documents.getById(eligible[i].id)
        if (updated) setDocs((prev) => prev.map((d) => (d.id === updated.id ? updated : d)))
      } catch {
        // continue on error for individual docs
      }
    }
    setBulkProgress(null)
  ...
  const extractableCount = doneCount
  const analyzableCount  = docs.filter((d) => d.processing_status === 'done' && d.word_count > 0).length
  const bulkBusy = processingAll || extractingAll || analyzingAll || processingCount > 0
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        {(pendingCount > 0 || extractableCount > 0 || analyzableCount > 0) && (
          <div className="flex flex-col items-end gap-2 shrink-0">
            {/* Bulk progress indicator */}
            {bulkProgress && (
              <p className="text-xs text-slate-400">
                {extractingAll ? 'Extracting vocab' : 'Analyzing readiness'} {bulkProgress.current}/{bulkProgress.total}…
              </p>
            )}
            <div className="flex gap-2">
              {pendingCount > 0 && (
                <Button onClick={processAll} variant="primary" size="sm" disabled={bulkBusy}>
                  {processingAll ? 'Running OCR…' : `OCR ${pendingCount} file${pendingCount !== 1 ? 's' : ''}`}
                </Button>
              )}
              {extractableCount > 0 && (
                <Button onClick={extractAllVocab} variant="secondary" size="sm" disabled={bulkBusy}>
                  {extractingAll ? `Extracting ${bulkProgress?.current}/${bulkProgress?.total}…` : `Extract Vocab (${extractableCount})`}
                </Button>
              )}
              {analyzableCount > 0 && (
                <Button onClick={analyzeAllReadiness} variant="ghost" size="sm" disabled={bulkBusy}>
                  {analyzingAll ? `Readiness ${bulkProgress?.current}/${bulkProgress?.total}…` : `Readiness (${analyzableCount})`}
                </Button>
              )}
            </div>
          </div>
