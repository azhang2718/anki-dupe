      ...
      .prepare(
        `INSERT INTO words
          (chinese, pinyin, meaning, difficulty, frequency_score, importance_score,
           part_of_speech, example_sentence, example_translation, source_document_id, category)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      )
      .run(
        word.chinese,
      ...
      getDb()
        .prepare(
          `UPDATE words SET pinyin=?, meaning=?, difficulty=?, frequency_score=?,
           importance_score=?, part_of_speech=?, example_sentence=?, example_translation=?, category=?
           WHERE chinese=?`
        )
        .run(
          word.pinyin,
